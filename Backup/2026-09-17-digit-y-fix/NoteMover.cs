...
public class NoteMover : MonoBehaviour
{
    ...
    [Tooltip("数字悬浮的世界坐标 Y（不是局部值，直接填场景里想要的高度，默认 1.63）。创建时按音符实际世界位置反算局部偏移。可在运行时拖。")]
    public float digitWorldY = 1.63f;
    ...
    public void RegisterChainTapHit(int remaining, int required, float songTime)
    {
        ...
        SetDigit(remaining);   // 命中显示当前数（非命中态；命中切 Select 见块 C）
    ...
    private void CreateDigitSprites(int count)
    {
        ...
        // 数字高度用世界坐标 digitWorldY：按音符实际世界 Y 与 lossyScale.y 反算局部偏移（音符 Y 恒定，创建时算一次即可）
        float lossyY = Mathf.Max(0.0001f, transform.lossyScale.y);
        float localY = (digitWorldY - transform.position.y) / lossyY;
        digitRoot.transform.localPosition = new Vector3(0f, localY, 0f);
