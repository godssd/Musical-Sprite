...
public class HoldNote : MonoBehaviour
{
    ...
    [Tooltip("链接带端点内缩比例（相对节点贴图实际半宽）：1=带子刚好从节点贴图外沿开始/结束，0=贴到节点中心。")]
...
// 所有节点都缩没后才销毁对象（期间其余节点继续逐片消失）
    ...
    /// <summary>进度线 marker：按 slideJudgment 原始尺寸 1:1，置于当前进度点；保底债务时相对判定线回退；
    /// 仅 Holding 时显示，末段接近完成(>0.85)淡出，避免与尾节点叠挤。</summary>
    private void UpdateProgressMarker()
    {
        if (progressMarker == null) return;
        float st = conductor != null ? conductor.songPosition : 0f;
        int lastSeg = bandMeshes.Count - 1;
        bool show = (state == HoldState.Holding && !finished);
        if (show && state == HoldState.Holding && lastSeg >= 0)
        {
            int curSeg = CurrentSegmentIndex(st);
            if (curSeg >= lastSeg)
            {
                float endT0 = times[curSeg], endT1 = times[curSeg + 1];
                float cov = Mathf.Clamp01((st - endT0) / Mathf.Max(endT1 - endT0, 0.0001f));
                if (cov > 0.85f) show = false; // 接近完成终点隐藏，避免与尾节点叠挤形变
            }
        }
