...
// 所有节点都缩没后才销毁对象（期间其余节点继续逐片消失）
    ...
    private void UpdateBandAll(float revealX, float judgeX)
    {
        ...
        // 覆盖带跟随进度线 markerDist：黄色(Select)覆盖「判定线内已过去部分 + 判定线外 0..cover」，其余为 base
        float cover = (state == HoldState.Holding) ? markerDist : 0f;
        ...
        for (int s = 0; s < bandMeshes.Count; s++)
        {
            ...
            // overlay：覆盖段 = 该段与 od∈(-∞, cover] 的交集（od = 朝 spawn 侧距判定线距离；od<0 = 已越过判定线的"过去"部分，同样计入充能）
            float od0 = odSign * (a.x - judgeLineX);
            float od1 = odSign * (b.x - judgeLineX);
            float dA = Mathf.Min(od0, od1);
            float dB = Mathf.Min(cover, Mathf.Max(od0, od1));
            bool hasCov = (dB - dA) > 1e-4f && Mathf.Abs(od1 - od0) > 1e-5f;
            if (hasCov)
            {
                float uA = (dA - od0) / (od1 - od0);
    ...
    private void UpdateProgressMarker()
    {
        ...
        // 进度线位置由 markerDist 推出：沿链体(朝 spawn 侧)距判定线 markerDist 的世界点
        Vector3 mp = ChainPointAtOutsideDist(markerDist);
        progressMarker.position = new Vector3(mp.x, rideY, mp.z);
    }
    ...
    private void Judge()
    {
        ...
        if (state == HoldState.Holding)
        {
            ...
            // 进度线 markerDist 速度模型（gap 域，世界距判定线距离）：<0.5 按住 1.5× 恢复 / 松手固定衰减；≥0.5 按住钉死 0.5(相对链=1×) / 松手衰减；<0→Miss。保底不因命中重置。
            ...
            float vDecay = 0.5f / Mathf.Max(breakThreshold, 0.0001f); // 衰减速率：0.5u / 保底时长
            float dt = Time.deltaTime;
            if (markerDist < markerHoldOffset)
            {
                // < 0.5：按住以 1× 音符速度恢复，松手按固定速率衰减（保底时间不被命中重置）
                markerDist += (held ? normalSpeed : -vDecay) * dt;
                markerDist = Mathf.Min(markerDist, markerHoldOffset);
            }
            else
            {
                // ≥ 0.5 且按住：钉死判定线外 markerHoldOffset(世界不动)。相对链体扫过速度自动=音符速度(1×)，即"相对位置不变"。
                // 松手：以 vDecay 朝判定线衰减；因 markerDist 恒 ≤ markerHoldOffset，衰减到线恰好 = breakThreshold 秒。
                if (held) markerDist = markerHoldOffset;
                else markerDist -= vDecay * dt;
            }
            if (markerDist < 0f) { Break(); return; } // 越过判定线(身后) → 断连 Miss
    ...
    public void StartHold(float songTime, bool fromAI)
    {
        ...
        markerDist = 0f;            // 进度线从判定线起（不越内），按住即 2× 上爬 → 解决起始滞后
