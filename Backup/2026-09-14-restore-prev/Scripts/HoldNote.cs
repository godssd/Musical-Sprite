...
public class HoldNote : MonoBehaviour
{
    ...
    public float visualScaleMul = 0.55f;     // 节点/连线视觉缩放（约一半）
    public float laneFit = 0.9f;             // 单节点跨轨方向(Z)占轨道比例上限
    ...
    private static readonly Color SegNormalColor = new Color(1f, 0.82f, 0.40f, 0.35f);  // 普通连线暖黄柔光
    private static readonly Color SegLitColor = new Color(1f, 0.95f, 0.72f, 0.60f);    // 已划过/点亮更亮
    ...
    private float hitPopScale = 1.2f;
    ...
    private void BuildVisuals()
    {
        ...
        float d = noteRadius * 2f * visualScaleMul;
        ...
        for (int i = 0; i < nodeCount; i++)
        {
            ...
            if (span > 1)
            {
                nodeZ = laneSpacing * (span - 1) + noteRadius * 2f * visualScaleMul;   // 跨 2 轨（Z 方向）
                nodeX = d;                                                              // 沿轨（X）保持单音符长度
            }
            else
            {
                float zMax = laneSpacing * laneFit;
                nodeZ = Mathf.Min(d, zMax);
                nodeX = nodeZ / nodeAspect;                                            // 保持原图比例
            }
            Vector3 nodeScale = SpriteScaleForSize(nodeSpr, nodeX, nodeZ);
            ...
            if (ag != null)
            {
                ...
                ngsr.color = new Color(1f, 0.90f, 0.62f, 0.18f);   // 更淡
                ng.transform.localScale = Vector3.one * 1.1f;       // 相对父(父已带 nodeScale)
            }
    ...
    private void UpdateAllSegments()
    {
        for (int s = 0; s < segmentGroups.Count; s++)
        {
            ...
            int spanB = NodeSpan(s + 1);
            int segSpan = Mathf.Max(spanA, spanB);
            float width = laneSpacing * (segSpan - 1) + noteRadius * 2f * visualScaleMul;
