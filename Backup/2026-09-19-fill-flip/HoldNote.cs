...
public class HoldNote : MonoBehaviour
{
    ...
    [Tooltip("进度线相对 rideY 的抬升高度（世界单位）。层级要求：判定线最低、提示灯在其上、进度线最高——避免按住期间被判定线/提示灯遮挡。")]
    public float progressMarkerLift = 0.5f;
...
// 所有节点都缩没后才销毁对象（期间其余节点继续逐片消失）
    ...
    private void UpdateBandAll(float revealX, float judgeX)
    {
        ...
        // 液面区间模型 [coverLo, coverHi]（od = 朝 spawn 侧距判定线距离，od>0=spawn 侧剩余部分）：
        // 按住(Holding)：[markerDist, 链尾od] —— 前沿横杠钉在 markerDist(按住=0=判定线，始终垂直音轨线)，
        //   黄液从横杠向 spawn 侧填满整条剩余链带（到链尾自然端），零空档；
        // 松手衰退：markerDist<0 → 横杠随衰退退入判定线内（断连预警可见，Break 逻辑不变）；
        ...
        {
            coverLo = markerDist;
            coverHi = odSign * (NodeRawPos(nodeCount - 1).x - judgeLineX);  // 链尾（最后节点=最 spawn 端）的 od
        }
        ...
        for (int s = 0; s < bandMeshes.Count; s++)
        {
            ...
            // overlay：覆盖段 = 该段 od 范围与 [coverLo, coverHi] 的区间交
            float od0 = odSign * (a.x - judgeLineX);
            float od1 = odSign * (b.x - judgeLineX);
            float dA = Mathf.Max(Mathf.Min(od0, od1), coverLo);
            float dB = Mathf.Min(Mathf.Max(od0, od1), coverHi);
            bool hasCov = (dB - dA) > 1e-4f && Mathf.Abs(od1 - od0) > 1e-5f;
            if (hasCov)
            {
                float uA = (dA - od0) / (od1 - od0);
                float uB = (dB - od0) / (od1 - od0);
                // 前沿横杠(low 端=od=markerDist)强制竖直：x 钉同一 coverWorldX（按住=判定线，松手=线内衰退位）
                float targetX = judgeLineX + odSign * markerDist;
    ...
    private void UpdateProgressMarker()
    {
        ...
        // y 抬升：判定线最低、提示灯(renderQueue=3050)在其上、进度线最高——按住期间不被遮挡。Inspector 可调(progressMarkerLift)
