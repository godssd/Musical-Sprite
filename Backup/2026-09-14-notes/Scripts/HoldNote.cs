...
public class HoldNote : MonoBehaviour
{
    ...
    private const int BarSegmentCount = 28;
    ...
    private float protectionMaxOffset = 0.32f;
    ...
    private void BuildVisuals()
    {
        ...
        for (int i = 0; i < nodeCount; i++)
        {
            ...
            Vector3 nodeScale = new Vector3(span > 1 ? noteRadius * 2f : r, span > 1 ? linkedWidth : r, 1f);
            ...
            sr.sprite = DefaultNode ?? RuntimeSpriteUtility.CreateRoundedBarSprite(32, 32, Color.white, Color.clear, 0, 8);
            sr.sortingOrder = 10;
            ...
            if (ag != null)
            {
                ...
                ngsr.color = new Color(1f, 0.85f, 0.4f, 0.45f);
                ng.transform.localScale = Vector3.one * 1.25f;   // 相对父(父已带 nodeScale)
            }
        ...
        for (int s = 0; s < nodeCount - 1; s++)
        {
            ...
            var segRends = new List<SpriteRenderer>();
            segmentBaseScales[s] = new Vector3[BarSegmentCount];
    ...
    private void CreateProtectionBar()
    {
        ...
        protectionBarRend.color = new Color(1f, 0.85f, 0.45f, 0.5f);   // 更淡
        protectionBarTr.localScale = new Vector3(0.035f, 0.25f, 1f);  // 更短
    ...
    private void UpdateSegmentColors(float songTime)
    {
        ...
        {
            var rends = segmentRendGroups[s];
            if (s < completedSegments)
            {
                for (int k = 0; k < rends.Count; k++)
                    if (rends[k] != null) rends[k].sprite = SelectLink;
                continue;
            }
            if (s > completedSegments)
            {
                for (int k = 0; k < rends.Count; k++)
                    if (rends[k] != null) rends[k].sprite = DefaultLink;
                continue;
            }
            var segs = segmentGroups[s];
            for (int k = 0; k < segs.Count; k++)
            {
                ...
                rends[k].sprite = passed ? SelectLink : DefaultLink;
            }
        }
    }
        ...
        for (int s = 0; s < segmentGroups.Count; s++)
        {
            ...
            for (int k = 0; k < segs.Count; k++)
            {
                ...
                if (skillCleared)
                {
                    ...
                    rends[k].sprite = SelectLink;
                    float fa = 1f - Mathf.Clamp01(fadeTimer / fadeDuration);
                    rends[k].color = new Color(1f, 1f, 1f, fa);
                    continue;
                }
                if (totalLen < 0.001f)
                {
                ...
                float halfLenX = segs[k].localScale.x * 0.5f * Mathf.Abs(normDir.x) + segs[k].localScale.y * 0.5f * Mathf.Abs(normDir.z);
                ...
                {
                    rends[k].enabled = true;
                    if (!missMode) rends[k].color = Color.white;
                }
                else
                {
                    ...
                    if (!missMode) rends[k].sprite = SelectLink;
                    rends[k].color = new Color(1f, 1f, 1f, 1f - f);
    ...
    private void ResetAllToDefault()
    {
        ...
        for (int s = 0; s < segmentRendGroups.Count; s++)
            for (int k = 0; k < segmentRendGroups[s].Count; k++)
                if (segmentRendGroups[s][k] != null) segmentRendGroups[s][k].sprite = DefaultLink;
    }
    ...
    private void Complete()
    {
        ...
        for (int s = 0; s < segmentRendGroups.Count; s++)
            for (int k = 0; k < segmentRendGroups[s].Count; k++)
                if (segmentRendGroups[s][k] != null) segmentRendGroups[s][k].sprite = SelectLink;
    }
}
