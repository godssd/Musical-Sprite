...
public class NoteMover : MonoBehaviour
{
    ...
    public float glowScaleMul = 1.35f;                            // 光晕相对主体的放大倍率
    public Color glowColor = new Color(1f, 0.92f, 0.65f, 1f);     // 光晕颜色（暖白）
    [Range(0f, 1f)] public float glowAlpha = 0.5f;                // 光晕强度
                     ...
                     bool isChainTap = false, int chainTapCount = 0, float chainTapHoldDuration = 0.4f)
    {
        ...
        // 计算基础缩放：X=飞行方向宽度，Y=视觉高度（world Z）
        float desiredWidth = noteRadius * 2f;
        float desiredHeight = desiredWidth;
        if (laneSpan > 1)
            desiredHeight = laneSpacing * (laneSpan - 1) + noteRadius * 2f;
    ...
    private void CreateChainCountSprite(int count)
    {
        ...
        if (ag != null)
        {
            ...
            digitGlowRend.color = new Color(glowColor.r, glowColor.g, glowColor.b, glowAlpha * 0.8f);
    ...
    private void SpawnBeam()
    {
        ...
        sr.color = new Color(1f, 0.95f, 0.8f, 0.9f);
        sr.sortingOrder = 13;
        sr.shadowCastingMode = UnityEngine.Rendering.ShadowCastingMode.Off;
        // 光束竖直向上（世界 +Y），宽度与音符相当；Wide 命中时光束更宽
        float beamW = baseScale.x * 0.6f * (laneSpan > 1 ? 1.6f : 1f);
        Vector3 beamScale = SpriteScaleForSize(beamSpr, beamW, baseScale.x * 3f);
