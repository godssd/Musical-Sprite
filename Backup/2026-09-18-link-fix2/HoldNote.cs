...
// 所有节点都缩没后才销毁对象（期间其余节点继续逐片消失）
    ...
    private void UpdateBandAll(float revealX, float judgeX)
    {
        ...
        // 完成态(state==Done 且未断连，含技能清屏 skillCleared)：cover=+inf → 整条连接带显示 Select 完成图（白），
        // 修复"clear 不切完成图"。漏击/断连(missMode)不显示覆盖带（恢复 α=0.6 半透路径）。
        bool completed = (state == HoldState.Done && !broken);
        float cover = holdingNow ? markerDist : (completed ? float.PositiveInfinity : float.NegativeInfinity);
