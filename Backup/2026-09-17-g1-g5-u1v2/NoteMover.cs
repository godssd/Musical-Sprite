...
public class NoteMover : MonoBehaviour
{
    ...
    // 通用命中/Miss 反馈（U1/U2）：命中后不冻结 Movement，由 Update 按越判定线距离驱动缩放/透明度
    private bool hitFeedbackActive = false;   // 命中反馈进行中（替代旧停顿消失）
    ...
    [Header("通用命中反馈 U1")]
    [Tooltip("命中'变大'段峰值缩放（默认 1.3）。全程不超正常速度，仅视觉放大。")]
    public float hitDecelScale = 1.3f;
    [Tooltip("越判定线缩小到的最小缩放（完全消失前，默认 0.75 ≈ 缩小 25%）。")]
    public float hitShrinkMin = 0.75f;
    [Tooltip("越判定线后渐隐行程（世界单位，默认 1u）；走完即销毁/出 MISS。")]
    ...
    /// <summary>U1 命中反馈：继续前进，命中瞬间为放大峰值，越判定线后缩小 + 渐隐，hitFadeDist 处销毁。</summary>
    private void ApplyHitFeedback()
    {
        float past = note.side == 0 ? (hitPos.x - transform.position.x) : (transform.position.x - hitPos.x);
        float scaleMul, alpha;
        if (past <= 0f)
        {
            // 命中瞬间 ~ 越过判定线前：越靠近判定线越大（命中峰值在判定线上），表达"减速"观感
            float g = Mathf.Clamp01((-past) / Mathf.Max(noteHalfSize, 0.001f)); // 0=判定线,1=远处
            scaleMul = Mathf.Lerp(hitDecelScale, 1f, g);
            alpha = 1f;
        }
        else
        {
            float f = Mathf.Clamp01(past / hitFadeDist);
            scaleMul = Mathf.Lerp(hitDecelScale, hitShrinkMin, f);
            alpha = 1f - f;
        }
