...
/// <summary>
/// 世界空间能量槽（2026-09-14 用户对齐方案，替换旧的 ScreenSpaceOverlay EnergyBarUIController）。
///
/// 【方案要点】
/// - 浮在角色 cube 脚下（世界空间、平躺 -90°X 面朝上方，俯视相机可见），每帧跟随对应 CharacterCubeMarker。
...
public class EnergyBarWorldSpace : MonoBehaviour
{
    ...
    public float feetYOffset = 0.03f;
    public bool useRendererBounds = false;     // Spine 接入后 cube 渲染器被禁用，默认关闭，改用 markerFootYOffset
    public float markerFootYOffset = 0f;       // 从 marker 世界 Y 到能量槽中心的偏移（marker 在脚底时给 0；负=下沉，正=抬高）
    ...
    private void BuildForCharacter(CharacterClass c, int side, int lane)
    {
        ...
        Transform marker = FindMarker(side, lane);
        ...
        if (marker != null) root.transform.position = new Vector3(marker.position.x, FeetWorldY(marker, feetYOffset), marker.position.z);
        ...
        for (int s = 0; s < c.maxEnergies.Length; s++)
        {
            ...
            float xOff = isDual
                ...
                : 0f;
            bars.Add(new WorldEnergyBar(this, c, side, lane, s, isDual, theme, root, xOff));
        }
    }
    ...
    /// <summary>把能量条压到角色脚下：Spine 接入后 cube 渲染器被禁用，回退到 markerFootYOffset（默认 0 = 落在 marker 高度）。</summary>
    private float FeetWorldY(Transform marker, float offset)
    {
        if (marker == null) return offset;
        if (useRendererBounds)
        {
            var r = marker.GetComponent<Renderer>();
            if (r != null && r.enabled) return marker.position.y - r.bounds.extents.y + offset;
        }
        return marker.position.y + markerFootYOffset + offset;
    }
    ...
    private class WorldEnergyBar
    {
        ...
        public WorldEnergyBar(EnergyBarWorldSpace mgr, CharacterClass owner, int side, int lane,
                              int slot, bool isDual, EnergyTheme theme, GameObject rootGo, float xOff)
        {
            ...
            this.marker = EnergyBarWorldSpace.FindMarker(side, lane);
        ...
        {
            if (root == null) return;
            if (marker == null) marker = EnergyBarWorldSpace.FindMarker(side, lane);
            if (marker == null) { root.SetActive(false); return; }
            root.SetActive(true);
            Vector3 target = new Vector3(marker.position.x, mgr.FeetWorldY(marker, mgr.feetYOffset), marker.position.z);
            root.transform.position = Vector3.Lerp(root.transform.position, target, Mathf.Min(1f, mgr.followLerp * Time.deltaTime));
        }
