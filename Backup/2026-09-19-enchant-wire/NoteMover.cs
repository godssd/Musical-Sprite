...
public class NoteMover : MonoBehaviour
{
    ...
    // 附魔换皮（同体换皮，几何不变）：被主动技能附魔时由 TintCharmed -> ApplyEnchantSkin 设置
    private bool _charmed = false;                      // 是否已应用附魔皮肤
    private Sprite _charmRepeatSelect;                 // 连点命中态贴图（技能专属）
    private Sprite[] _charmDigits = new Sprite[10];    // 连点数字普通态（技能专属）
    private Sprite[] _charmDigitsSelect = new Sprite[10]; // 连点数字命中态（技能专属）
    ...
    // 通用命中/Miss 反馈（U1/U2）：命中先停顿（切 Select + 变大），随后恢复常速前进 + 缩小 + 渐隐
    private bool hitFeedbackActive = false;   // 命中反馈进行中
    private Vector3 hitBaseScale;             // 命中瞬间基准缩放
    private Vector3 missBaseScale;            // 漏击瞬间基准缩放
    private bool skillClearPlaying = false;   // 技能清屏：保持原位放大淡出（冻结 Movement，沿用旧表现）
    private bool hitPaused = false;           // 是否已进入停顿期（首帧记录停顿位置/结束时刻）
    private float hitPauseEnd = 0f;           // 停顿结束的 songTime
    private Vector3 hitPausePos = Vector3.zero; // 停顿期保持的位置
    [Header("通用命中反馈（改回：命中变大直接消失，不前进/不缩小）")]
    [Tooltip("命中'变大'段峰值缩放（默认 1.4）。")]
    public float hitDecelScale = 1.4f;
    [Tooltip("命中后钉在原地：scale 1→hitDecelScale 同时 alpha 1→0，时长（秒，默认 0.35），到时销毁。")]
    public float hitFadeDur = 0.35f;
    // 以下参数为旧 U1-v2（停顿→前进缩小渐隐）遗留，已弃用，保留以防旧存档引用
    [HideInInspector] public float hitStopDur = 0.18f;
    [HideInInspector] public float hitShrinkMin = 0.75f;
    [HideInInspector] public float hitFadeDist = 1.0f;
    [Header("过粉杠渐显")]
    [Tooltip("音符越过粉杠（中线）后出现的不透明度渐显时长（秒，默认 0.2）。硬切改为渐显。")]
    public float revealFadeDur = 0.2f;
    private float revealAlpha = 1f;   // 过粉杠渐显系数（0→1），与命中/Miss 消失系数相乘
    ...
    void Update()
    {
        ...
        if (skillClearPlaying) return; // 技能清屏：保持原位放大淡出（沿用旧表现，冻结 Movement）

        // 连点命中后：先匀减速后退 chainRetreatDist（chainTapHoldDuration 内速度降到 0），
        // 再以正常前进速度 normalSpeed 朝判定线回来；重新接触判定线附近可点击，越过则 Miss。
        // 命中反馈（hitFeedbackActive）/ 漏击（missing）启动后让位给普通移动 + 渐隐逻辑。
        if (isChainTap && chainTapDeadline >= 0f && !hitFeedbackActive && !missing)
        {
        ...
        // 命中反馈（U1-v2）期间由 ApplyHitFeedback 接管移动（先停顿后恢复前进），这里跳过
        ...
        if (shouldBeVisible && !note.isVisible)
        {
            ...
            revealStart = Time.time; // 开始渐显（替代原硬切 SetAlpha(1)）
        }
        ...
        // 过粉杠渐显系数（硬切 → 0→1 渐显），与命中/Miss 消失系数相乘
        if (revealStart >= 0f)
            revealAlpha = Mathf.Clamp01((Time.time - revealStart) / Mathf.Max(revealFadeDur, 0.0001f));

        // U1/U2 通用反馈：命中后继续前进 + 越判定线渐隐；漏击继续前进 + 缩小渐隐。
        // （Movement 已在上方按歌曲时间推进，这里只控制缩放/透明度/销毁时机）
        if (hitFeedbackActive) ApplyHitFeedback();
        else if (missing) ApplyMissFeedback();
        else if (note.isVisible && noteMaterial != null)
        {
            // 非命中/Miss 期间：仅按渐显系数呈现（revealAlpha）
            Color c = GetNoteTint();
            c.a = revealAlpha;
            SetNoteTint(c);
            SetDigitAlpha(revealAlpha);
        }
    }

    /// <summary>
    /// 播放命中反馈：切 Select 贴图 + 触发 ApplyHitFeedback（钉住原地，变大同时渐隐消失）。
    /// 缩放/透明度由 Update 每帧的 ApplyHitFeedback 驱动，Movement 在命中期间冻结（钉死）。
    /// </summary>
    public void PlayHitAnimation(string rank)
    {
        ...
        hitPaused = false;               // 下一帧 ApplyHitFeedback 记录停顿位置/结束时刻
        hitBaseScale = transform.localScale;
        if (noteMaterial != null)
        {
            if (textured && _selectTex != null)
            {
                ApplySelectTexture();      // 换成"完成"贴图，以原色显示
                SetNoteTint(Color.white);
            }
            else
            {
                SetNoteTint(Color.black);  // 旧黑底：保持变黑强调
            }
        }
    }

    /// <summary>
    /// 进入漏击状态（U2）：继续前进（匀速），缩小到 hitShrinkMin + 渐隐，越线 hitFadeDist 后
    /// 由 NoteSpawner 出 MISS 反馈并销毁（missComplete 暴露，不在此直接 Destroy）。
    /// </summary>
    public void BeginMiss()
    {
        ...
        animationPlaying = true;
        StopAllCoroutines();
        missBaseScale = transform.localScale;
    }
    ...
    private void SetDigit(int n)
    {
        ...
        for (int slot = 0; slot < 2; slot++)
        {
            if (slot < figs.Length)
            {
                Sprite sp = CharmDigit(figs[slot]);
    ...
    private void SwapDigitToSelect()
    {
        ...
        for (int slot = 0; slot < digitFiguresCurrent.Length; slot++)
        {
            ...
            Sprite sp = CharmDigitSelect(digitFiguresCurrent[slot]);
            if (sp != null) digitMats[slot].SetTexture(TexturePropName(digitMats[slot]), sp.texture);
        }
    }

    /// <summary>附魔换皮时取技能专属数字贴图，否则回退 NoteSpriteLibrary 基础数字。</summary>
    private Sprite CharmDigit(int d)
    {
        if (_charmed && _charmDigits != null && d >= 0 && d < _charmDigits.Length && _charmDigits[d] != null)
            return _charmDigits[d];
        return (NoteSpriteLibrary.Instance != null) ? NoteSpriteLibrary.Instance.repeatDigits[d] : null;
    }

    /// <summary>附魔换皮时取技能专属数字命中态贴图，否则回退基础数字 Select。</summary>
    private Sprite CharmDigitSelect(int d)
    {
        if (_charmed && _charmDigitsSelect != null && d >= 0 && d < _charmDigitsSelect.Length && _charmDigitsSelect[d] != null)
            return _charmDigitsSelect[d];
        return (NoteSpriteLibrary.Instance != null) ? NoteSpriteLibrary.Instance.repeatDigitsSelect[d] : null;
    }
    ...
    private void ApplySelectTexture()
    {
        ...
        if (isChainTap)
        {
            // 连点：体切 附魔命中态(若有) 否则 repeatSelect（完成体）；数字切技能专属数字 Select
            if (_charmed && _charmRepeatSelect != null)
                noteMaterial.SetTexture(texProp, _charmRepeatSelect.texture);
            else if (NoteSpriteLibrary.Instance != null && NoteSpriteLibrary.Instance.repeatSelect != null)
    ...
    /// <summary>
    /// 附魔换皮入口：被主动技能附魔时由 NoteSpawner.TintCharmed 调用（已用 charmOwner.skillId 查到对应皮肤）。
    /// 按音符类型把材质 _BaseMap 换成技能专属普通态贴图；命中态贴图记录到 _selectTex / _charmRepeatSelect；
    /// 连点数字同步切到技能专属数字。几何与 Mesh 不变，仅同体换皮。
    /// </summary>
    public void ApplyEnchantSkin(EnchantSkin skin)
    {
        if (skin == null || noteMaterial == null) return;
        Sprite normal = null, select = null;
        if (isChainTap)
        {
            normal = skin.repeat; select = skin.repeatSelect;
            _charmRepeatSelect = skin.repeatSelect;
            _charmDigits = skin.repeatDigits; _charmDigitsSelect = skin.repeatDigitsSelect;
        }
        else if (isWideTap)       { normal = skin.wide;     select = skin.wideSelect; }
        else if (isSmallTap)      { normal = skin.smallTap; select = skin.smallTapSelect; }
        else                      { normal = skin.tap;      select = skin.tapSelect; }

        if (normal == null) return;
        string texProp = noteMaterial.HasProperty("_BaseMap") ? "_BaseMap" : (noteMaterial.HasProperty("_MainTex") ? "_MainTex" : "_BaseMap");
        noteMaterial.SetTexture(texProp, normal.texture);
        _baseTex = normal.texture;
        _selectTex = (select != null) ? select.texture : null;
        _charmed = true;
        if (isChainTap && digitDisplay >= 0) SetDigit(digitDisplay); // 立即把数字切到技能专属数字
    }

    /// <summary>
    /// 通用命中反馈（改回"命中变大直接消失"）：命中瞬间钉住位置，scale 1→hitDecelScale 同时
    /// alpha 1→0（hitFadeDur 秒），位置不动、不前进、不缩小，到时销毁。
    /// 移动由 Update 在命中期间跳过线性插值（上方 if(!hitFeedbackActive) 守卫）保证钉死。
    /// </summary>
    private void ApplyHitFeedback()
    {
        float songPos = conductor != null ? conductor.songPosition : 0f;

        if (!hitPaused)
        {
            hitPaused = true;
            hitPauseEnd = songPos + hitFadeDur;
            hitPausePos = transform.position;   // 钉在命中位置
        }

        float k = Mathf.Clamp01((songPos - (hitPauseEnd - hitFadeDur)) / Mathf.Max(hitFadeDur, 0.0001f));
        float s = Mathf.Lerp(1f, hitDecelScale, k);            // 变大
        float alpha = (1f - k) * revealAlpha;                  // 渐隐
        transform.position = hitPausePos;                      // 不动
        transform.localScale = hitBaseScale * s;
        if (noteMaterial != null) { Color c = GetNoteTint(); c.a = alpha; SetNoteTint(c); }
        SetDigitAlpha(alpha);
        if (k >= 1f) Destroy(gameObject);
    }

    /// <summary>U2 漏击反馈：继续前进（匀速），缩小到 hitShrinkMin + 渐隐，越线 hitFadeDist 后交由 NoteSpawner 出 MISS。</summary>
    private void ApplyMissFeedback()
    {
        float past = note.side == 0 ? (hitPos.x - transform.position.x) : (transform.position.x - hitPos.x);
        float f = Mathf.Clamp01(past / hitFadeDist);
        float scaleMul = Mathf.Lerp(1f, hitShrinkMin, f);
        float alpha = 1f - f;
        transform.localScale = missBaseScale * scaleMul;
        if (noteMaterial != null) { Color c = GetNoteTint(); c.a = alpha * revealAlpha; SetNoteTint(c); }
        SetDigitAlpha(alpha * revealAlpha);
        if (past >= hitFadeDist) missComplete = true; // 交给 NoteSpawner 出 MISS 反馈并销毁
    }
    ...
    /// <summary>
    /// ③ 断弦高压（清屏）对连点音符的逐音符反馈入口：冻结移动，播放一次"命中变大切图"反馈，
    /// 随后按剩余次数做 YES 错峰弹数（每 0.1s 一个，防堆叠）。
    /// 规则（用户拍板）：
    /// - 剩余 &gt; 5：技能"提前完成 5 次"（不额外加分），做 5 个 YES，剩余仍 &gt;0 则解冻交回玩家继续；
    /// - 剩余 ≤ 5：做"剩余个"YES + 末位 CLEAR，整条完成；
    /// - 只剩 1：直接命中反馈（最后）→ CLEAR 收尾。
    /// 所有技能驱动的 YES/CLEAR 仅走 JudgeFeedbackManager 视觉，不走 ReportJudge 计分管线（不额外加分）。
    /// </summary>
    public void SkillDriveChainTap(ActiveSkillRuntime caster, NoteSpawner spawner)
    {
        if (caster == null || note == null || note.isHit || skillClearPlaying) return;
        StartCoroutine(SkillDriveChainTapCoroutine(caster, spawner));
    }

    /// <summary>技能清屏：冻结/解冻本音符的移动（Update 早退）。</summary>
    public void SetSkillFrozen(bool frozen)
    {
        skillClearPlaying = frozen;
    }

    private System.Collections.IEnumerator SkillDriveChainTapCoroutine(ActiveSkillRuntime caster, NoteSpawner spawner)
    {
        skillClearPlaying = true; // 冻结 Movement（Update 早退，音符停在判定线处）

        int remaining = note.chainTapRemaining;
        if (remaining <= 0)
        {
            FinishSkillChainTap(true);
            yield break;
        }

        // 只剩 1：直接命中反馈（最后）→ CLEAR 收尾（大白消失）
        if (remaining <= 1)
        {
            var jfm0 = FindFirstObjectByType<JudgeFeedbackManager>();
            if (jfm0 != null) jfm0.ShowFeedback(note.side, note.lane, "CLEAR", transform.position, note);
            FinishSkillChainTap(true);
            yield break;
        }

        // 剩余 &gt; 1：一次命中反馈（非最后）：体 + 数字切 Select，放大弹跳
        ApplySelectTexture();
        SwapDigitToSelect();
        SetChainBodyColor(Color.white);
        yield return StartCoroutine(ChainTapHitPopCo()); // ~0.18s 弹跳

        // YES 阶梯：技能"提前完成" batch 次（不额外加分，仅视觉）
        int batch = (remaining > 5) ? 5 : remaining;
        var jfm = FindFirstObjectByType<JudgeFeedbackManager>();
        for (int k = 0; k < batch; k++)
        {
            yield return new WaitForSeconds(0.1f); // YES 错峰 0.1s，防堆叠
            note.chainTapRemaining = Mathf.Max(0, note.chainTapRemaining - 1);
            SetDigit(note.chainTapRemaining); // 数字随剩余递减
            if (jfm != null) jfm.ShowFeedback(note.side, note.lane, "YES", transform.position, note);
        }

        if (note.chainTapRemaining <= 0)
        {
            // 末位 CLEAR：整条完成（大白消失）
            if (jfm != null) jfm.ShowFeedback(note.side, note.lane, "CLEAR", transform.position, note);
            FinishSkillChainTap(true);
        }
        else
        {
            // 剩余仍 &gt; 5：技能"提前完成 5 次"后解冻，交回玩家继续（不额外加分）
            if (spawner != null) spawner.ReAddChainTapNote(note);
            ResetChainTapForContinue();
            skillClearPlaying = false;
        }
    }

    /// <summary>技能驱动的连点音符收尾：标记命中、结算附魔、大白消失并销毁。</summary>
    private void FinishSkillChainTap(bool doSkillClearVisual)
    {
        if (note != null)
        {
            note.isHit = true;
            note.chainTapRemaining = 0;
            if (note.charmOwner != null) { note.charmOwner.OnCharmedNoteResolved(note, true); note.charmOwner = null; }
        }
        if (doSkillClearVisual) PlaySkillClear();
        else Destroy(gameObject);
    }

    /// <summary>技能"提前完成 5 次"后把连点音符复位到判定线、刷新可点击窗口，交回玩家继续。</summary>
    private void ResetChainTapForContinue()
    {
        ...
        chainTapDeadline = chainNextContactTime + goodWindow;
        chainFarthestDone = false;
        if (note != null) note.chainTapWaiting = false;
        transform.position = new Vector3(hitPos.x, rideY, hitPos.z);
        RevertSelectTexture();        // 体切回基准贴图（Note_Repeat）
        SetChainBodyColor(Color.white);
    }
