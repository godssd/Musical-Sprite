...
public class NoteMover : MonoBehaviour
{
    ...
    [Header("通用命中反馈 U1")]
    [Tooltip("命中后停顿时长（秒）：切 Select 贴图 + 变大，随后恢复常速前进。")]
    public float hitStopDur = 0.18f;
    [Tooltip("命中'变大'段峰值缩放（默认 1.4）。停顿末尾达到，恢复前进后回落到 hitShrinkMin。")]
    public float hitDecelScale = 1.4f;
    [Tooltip("恢复前进后缩小到的最小缩放（完全消失前，默认 0.75 ≈ 缩小 25%）。")]
    public float hitShrinkMin = 0.75f;
    [Tooltip("恢复前进后渐隐行程（世界单位，默认 1u）；走完即销毁/出 MISS。")]
    public float hitFadeDist = 1.0f;
    [Header("过粉杠渐显")]
    ...
    /// <summary>
    /// 播放命中反馈（U1）：不停顿、继续前进；变大段（峰值 hitDecelScale）在命中瞬间呈现，
    /// 随后缩小到 hitShrinkMin 并随越判定线距离渐隐（hitFadeDist 处销毁）。
    /// 缩放/透明度由 Update 每帧的 ApplyHitFeedback 驱动，Movement 不冻结。
    /// </summary>
    ...
    /// <summary>
    /// U1-v2 命中反馈：先停顿（切 Select 已在 PlayHitAnimation 完成 + 放大到峰值 hitDecelScale），
    /// 停顿结束恢复常速前进（缩小到 hitShrinkMin + 渐隐 hitFadeDist 后销毁）。
    /// 移动由本方法驱动（Update 在命中期间跳过线性插值）。
    /// </summary>
    private void ApplyHitFeedback()
    {
        float songPos = conductor != null ? conductor.songPosition : 0f;
        Vector3 fwd = new Vector3(note.side == 0 ? -1f : 1f, 0f, 0f); // 前进方向

        if (!hitPaused)
        {
            hitPaused = true;
            hitPauseEnd = songPos + hitStopDur;
            hitPausePos = transform.position;
        }

        if (songPos < hitPauseEnd)
        {
            // 停顿期：保持位置 + 放大到峰值
            float k = Mathf.Clamp01((songPos - (hitPauseEnd - hitStopDur)) / Mathf.Max(hitStopDur, 0.0001f));
            transform.position = hitPausePos;
            float s = Mathf.Lerp(1f, hitDecelScale, Mathf.Sin(k * Mathf.PI * 0.5f));
            transform.localScale = hitBaseScale * s;
            Color c = GetNoteTint(); c.a = revealAlpha; SetNoteTint(c);
            SetDigitAlpha(revealAlpha);
            return;
        }

        // 恢复常速前进 + 缩小 + 渐隐
        float adv = normalSpeed * (songPos - hitPauseEnd);
        Vector3 p = hitPausePos + fwd * adv;
        p.y = rideY;
        transform.position = p;
        float past = note.side == 0 ? (hitPos.x - p.x) : (p.x - hitPos.x);
        float fade = Mathf.Clamp01(past / hitFadeDist);
        float scaleMul = Mathf.Lerp(hitDecelScale, hitShrinkMin, fade);
        float alpha = (1f - fade) * revealAlpha;
        transform.localScale = hitBaseScale * scaleMul;
        if (noteMaterial != null) { Color c = GetNoteTint(); c.a = alpha; SetNoteTint(c); }
        SetDigitAlpha(alpha);
        if (past >= hitFadeDist) Destroy(gameObject);
    }
