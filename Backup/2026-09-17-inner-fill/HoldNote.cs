...
// 所有节点都缩没后才销毁对象（期间其余节点继续逐片消失）
    ...
    private void UpdateBandAll(float revealX, float judgeX)
    {
        ...
        // 覆盖带跟随进度线 markerDist：黄色(Select)覆盖「判定线外 0..cover」这段，其余为 base
        ...
        for (int s = 0; s < bandMeshes.Count; s++)
        {
            ...
            // overlay：覆盖段 = 该段与 od∈[0, cover] 的交集（od = 朝 spawn 侧距判定线距离）
            float od0 = odSign * (a.x - judgeLineX);
            float od1 = odSign * (b.x - judgeLineX);
            float dA = Mathf.Max(0f, Mathf.Min(od0, od1));
    ...
    private void Judge()
    {
        ...
        if (state == HoldState.Holding)
        {
            ...
            {
                // < 0.5：按住以 1.5× 速度恢复(防过快)，松手按固定速率衰减（保底时间不被命中重置）
                markerDist += (held ? 1.5f * normalSpeed : -vDecay) * dt;
