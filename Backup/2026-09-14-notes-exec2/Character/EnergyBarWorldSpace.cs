...
public class EnergyBarWorldSpace : MonoBehaviour
{
    ...
    /// <summary>把能量条压到角色脚下：取 marker 包围盒半高，回落到脚底 + offset。</summary>
    private static float FeetWorldY(Transform marker, float offset)
    {
        if (marker == null) return offset;
        float halfH = 0.5f;
        var r = marker.GetComponent<Renderer>();
        if (r != null) halfH = r.bounds.extents.y;
        return marker.position.y - halfH + offset;
    }
    ...
    private class WorldEnergyBar
    {
        ...
        public void Follow()
        {
            ...
            root.SetActive(true);
            Vector3 target = new Vector3(marker.position.x, FeetWorldY(marker, mgr.feetYOffset), marker.position.z);
