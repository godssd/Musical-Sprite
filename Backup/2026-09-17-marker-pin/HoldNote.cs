...
// 所有节点都缩没后才销毁对象（期间其余节点继续逐片消失）
    ...
    private void Judge()
    {
        ...
        if (state == HoldState.Holding)
        {
            ...
            // 进度线 markerDist 速度模型（判定线±0.5 健康 + 沿链推进；保底不因命中重置）
            ...
            {
                // < 0.5：按住以 2× 速度恢复，松手按固定速率衰减（保底时间不被命中重置）
                markerDist += (held ? 2f * normalSpeed : -vDecay) * dt;
                markerDist = Mathf.Min(markerDist, markerHoldOffset);
            }
            else
            {
                // ≥ 0.5：正确按住沿链以音符速度前进(相对音符位置不变=逐渐覆盖整条)；松手衰减
