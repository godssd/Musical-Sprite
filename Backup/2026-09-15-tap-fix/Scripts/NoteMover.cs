...
public class NoteMover : MonoBehaviour
{
    ...
    [Header("换皮（仅普通点击）")]
    [Tooltip("普通点击音符贴图；留空则尝试从 NoteSpriteLibrary.tap 读取。能取到即启用卡通换皮，且仅作用于普通点击，不影响小点击/跨轨/连点/按住等其他类型。")]
    public Texture2D tapTexture;
    [Tooltip("可选成品材质（需 URP Unlit/Transparent + _BaseMap）。留空则运行时用 tapTexture 自动生成。")]
    public Material tapNoteMaterial;

    [Header("普通点击视觉（仅普通点击，不影响其他类型与判定窗口）")]
    [Tooltip("普通点击整体缩放倍数；默认 0.687 ≈ 用户运行时把 scale.x 由 1.2 调到 0.824。只缩视觉，不改 noteRadius，判定窗口不变。")]
    public float tapVisualScaleMul = 0.687f;
                     ...
                     bool isChainTap = false, int chainTapCount = 0, float chainTapHoldDuration = 0.4f)
    {
        ...
        textured = isPlainTap && tapTex != null;
        ...
        // 运行时替换预制体网格：普通点击且换皮时用圆角平板当画布，连轨点击也用圆角平板；其余用圆柱。
        MeshFilter mf = GetComponent<MeshFilter>();
        if (mf != null) mf.sharedMesh = (laneSpan > 1 || textured) ? RoundedRectMesh : CylinderMesh;
        ...
        // 普通点击：仅视觉收窄（方法 B）——保持 X/Z 等比，不超当前音轨；只缩视觉，noteRadius 与判定窗口不变。
        if (isPlainTap)
        ...
        if (rend != null)
        {
            if (textured)
            {
                ...
                noteMaterial = tapNoteMaterial != null ? new Material(tapNoteMaterial) : MakeTapMaterial(tapTex);
    ...
    private Material MakeTapMaterial(Texture2D tex)
    {
        ...
        // 透明：交给 URP 自己按 _Surface/_Blend 配混合（不要手填 _SrcBlend/_DstBlend，值易错且会和 URP 内部冲突）。
        if (mat.HasProperty("_Surface")) { mat.SetFloat("_Surface", 1f); mat.EnableKeyword("_SURFACE_TYPE_TRANSPARENT"); }
        if (mat.HasProperty("_Blend")) mat.SetFloat("_Blend", 1f); // 1 = Alpha（标准半透明）
