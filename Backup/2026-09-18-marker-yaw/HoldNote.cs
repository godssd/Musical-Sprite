...
public class HoldNote : MonoBehaviour
{
    ...
    [Tooltip("进度线 marker 绕 Y 轴偏航角（度）。默认 90=长边平行判定线(沿 Z 轨道方向)，不再随链接线斜向旋转。")]
    public float progressMarkerYaw = 90f;
...
// 所有节点都缩没后才销毁对象（期间其余节点继续逐片消失）
    ...
    private void RebuildRibbon(int s, float u0, float u1, Mesh mesh, float judgeX, float cumStart, float totalLen, float yDip, float coverWorldX = float.NaN)
    {
    ...
    private void UpdateProgressMarker()
    {
        ...
        // ① 进度线强制平行判定线(沿 Z 轨道方向)，不随链接线斜向旋转；角度 Inspector 可调(progressMarkerYaw)
