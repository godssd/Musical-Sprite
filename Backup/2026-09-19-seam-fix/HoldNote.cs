...
// 所有节点都缩没后才销毁对象（期间其余节点继续逐片消失）
    ...
    private void RebuildRibbon(int s, float u0, float u1, Mesh mesh, float judgeX, float cumStart, float totalLen, float yDip, float edgeX0 = float.NaN, float edgeX1 = float.NaN)
    {
        ...
        // ① 端点竖切（替代脆弱的 coverWorldX 容差补丁）：u=0/u=1 端强制 x 为竖直值，保证带端永远垂直音轨线、与音符直边对齐，
        // 斜向/跨轨链接旋转时端边不露斜楔。overlay(黄液) 用传入 edgeX（液面竖直墙）；base 带用几何锚点 x（aEdge.x/bEdge.x，藏在音符下）。
        float vx0 = float.IsNaN(edgeX0) ? aEdge.x : edgeX0;
        float vx1 = float.IsNaN(edgeX1) ? bEdge.x : edgeX1;
