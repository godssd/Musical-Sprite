...
public class HoldNote : MonoBehaviour
{
    ...
    private float bandBreakDebt = 0f; // 危险回退量（0→0.5u）：正常按住=0（进度线在 markerHoldOffset 前方）；快断连时随 breakTimer 增大→进度线退向判定线示警
    ...
    [Tooltip("进度线(Holding 时)相对判定线固定的前方偏移（世界单位，默认 0.5）：正常按住时进度线钉在'判定线外 0.5'，不随节点飞走。")]
    ...
    [Tooltip("起手宽限（秒）：StartHold 后头 0.08s 内不累计断连计时，避免起手一两帧输入没跟上就误断连。")]
...
// 所有节点都缩没后才销毁对象（期间其余节点继续逐片消失）
    ...
    private void RebuildRibbon(int s, float u0, float u1, Mesh mesh, float judgeX, float cumStart, float totalLen, float yDip)
    {
        ...
        Vector3 aEdge = a + dir * halfA;
        Vector3 bEdge = b - dir * halfB;
        ...
        for (int i = 0; i <= M; i++)
        {
            ...
            Vector3 cL = p + new Vector3(0f, 0f, ww * 0.5f);
            Vector3 cR = p - new Vector3(0f, 0f, ww * 0.5f);
    ...
    private void ApplyBandAlpha(Mesh mesh, float revealX, float judgeX, float g)
    {
        ...
        {
            Vector3 vp = vs[2 * i];
            float rev = side == 0 ? Smoothstep(revealX + bandRevealWidth, revealX, vp.x) : Smoothstep(revealX - bandRevealWidth, revealX, vp.x);
            float fade = side == 0 ? Smoothstep(judgeX - bandFadeWidth, judgeX, vp.x) : Smoothstep(judgeX + bandFadeWidth, judgeX, vp.x);
    ...
    private void UpdateBandAll(float revealX, float judgeX)
    {
        ...
        {
            cum[s] = total;
            total += Vector3.Distance(nodeTransforms[s].position, nodeTransforms[s + 1].position);
        }

        float st = conductor != null ? conductor.songPosition : 0f;
        int curSeg = (state == HoldState.Holding) ? CurrentSegmentIndex(st) : -1;
        float cov = 0f;
        if (curSeg >= 0 && curSeg < bandMeshes.Count)
        {
            float t0 = times[curSeg], t1 = times[curSeg + 1];
            cov = Mathf.Clamp01((st - t0) / Mathf.Max(t1 - t0, 0.0001f));
        }

        for (int s = 0; s < bandMeshes.Count; s++)
        {
            Vector3 a = nodeTransforms[s].position, b = nodeTransforms[s + 1].position;
            bool revealed = IsBeyondLine(a.x, revealX) || IsBeyondLine(b.x, revealX);
            bandRends[s].enabled = revealed;

            bool done = s < completedSegments;
            bool future = (curSeg < 0) || (s > curSeg);
            bool overlayActive = false;
            if (done)
            {
                SetBandTexture(bandMats[s], slideLinkSelectTex);
            ...
            else // 当前进行段：cov 之前=Select 覆盖，之后=未完成 base
            {
                SetBandTexture(bandMats[s], slideLinkTex);
                if (cov >= 0.999f)
                {
                ...
                {
                    RebuildRibbon(s, cov, 1f, bandMeshes[s], judgeX, cum[s], total, bandRideYDip);
                    RebuildRibbon(s, 0f, cov, bandOverlayMeshes[s], judgeX, cum[s], total, bandRideYDip + 0.005f);
                    overlayActive = true;
                }
            }

            bandOverlayRends[s].enabled = revealed && overlayActive;
            ApplyBandAlpha(bandMeshes[s], revealX, judgeX, g);
            if (overlayActive) ApplyBandAlpha(bandOverlayMeshes[s], revealX, judgeX, g);
        }
    }

    /// <summary>进度线 marker：按 slideJudgment 原始尺寸 1:1。Holding 时 X 钉死在"判定线 + dir×markerHoldOffset"
    /// （默认判定线外 0.5），不再随飞行节点越跑越远；Z 用当前段插值（平滑跟随连轨横向移动）。
    /// 快断连时 bandBreakDebt 增大→进度线退向判定线示警。仅 Holding 且未完成时显示，完成(State.Done)自动隐藏。</summary>
    private void UpdateProgressMarker()
    {
        ...
        // Z 取当前段两端插值（连轨横向移动时平滑跟随）；X 钉死，不读飞行节点
        float z = Mathf.Lerp(nodeTransforms[seg].position.z, nodeTransforms[seg + 1].position.z, f);
        float dir = side == 0 ? -1f : 1f;
        float offset = Mathf.Max(0f, markerHoldOffset - bandBreakDebt); // 正常=0.5；快断连→退向判定线
        float x = judgeLineX + dir * offset;
        progressMarker.position = new Vector3(x, rideY, z);
    }
    ...
    private void Judge()
    {
        ...
        if (state == HoldState.Holding)
        {
            ...
            if (!isAI)
            {
                ...
                // 起手宽限：StartHold 后 holdStartGrace 内不累计断连，避免起手误断
                bool inGrace = (songTime - holdStartTime) < holdStartGrace;
                if (!held && !inGrace) breakTimer += Time.deltaTime;
                else breakTimer = 0f;
