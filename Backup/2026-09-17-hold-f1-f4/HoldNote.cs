...
public class HoldNote : MonoBehaviour
{
    ...
    [Tooltip("链接带一个贴图 tile 对应的世界长度（单位）。")]
    public float bandTileWorldLen = 1.0f;
    ...
    private void BuildVisuals()
    {
        ...
        if (slideLinkTex != null) slideLinkTex.wrapMode = TextureWrapMode.Repeat;
...
// 所有节点都缩没后才销毁对象（期间其余节点继续逐片消失）
    ...
    /// <summary>每帧按节点实时位置重建每段 ribbon 的几何 + UV（沿带长平铺 tile）。</summary>
    private void UpdateBandGeometry()
    {
        ...
        for (int s = 0; s < bandMeshes.Count; s++)
        {
            ...
            for (int i = 0; i <= M; i++)
            {
                ...
                if (i > 0) accum += Vector3.Distance(prev, p);
                prev = p;
                ...
                float uvx = accum / Mathf.Max(bandTileWorldLen, 0.0001f);
                uvs[2 * i] = new Vector2(uvx, 1f);
                uvs[2 * i + 1] = new Vector2(uvx, 0f);
    ...
    /// <summary>可见性 + 越判定线 smoothstep 软边消失 + 漏击/收尾整体透明度。</summary>
