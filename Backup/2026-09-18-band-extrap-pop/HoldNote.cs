...
// 所有节点都缩没后才销毁对象（期间其余节点继续逐片消失）
    ...
    private void UpdateBandAll(float revealX, float judgeX)
    {
        ...
        // 覆盖带跟随进度线 markerDist，且仅在 Holding（起始音符已命中）时充能：
        // 未命中/Waiting/Miss 后一律无黄色（修复"没点也充能"）。cover=markerDist，稳定=0（判定线）；
        // 松手衰退时 cover<0，黄色前沿退入判定线内（部分已过带子退黄）。非 Holding 时 cover=-∞ → 无任何覆盖。
        bool holdingNow = (state == HoldState.Holding);
        float cover = holdingNow ? markerDist : float.NegativeInfinity;
