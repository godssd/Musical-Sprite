...
cols = ["基础 (base)", "v7 (skill_3_howl)", "004 (skill_4)"]
...
for i, c in enumerate(cols):
    cx = ROWLABEL_W + PAD + i*CELL + CELL//2
    d.text((cx, HEADER_H//2), c, fill=(235,235,240), font=fh, anchor="mm")
d.line([(0, HEADER_H), (W, HEADER_H)], fill=(140,140,150), width=1)

for ri, (label, base, v7, ref) in enumerate(types):
    y = HEADER_H + PAD + ri*(CELL+PAD)
    # row label
    d.text((ROWLABEL_W//2, y+CELL//2), label, fill=(210,210,220), font=f, anchor="mm")
    # columns
    for i, fn in enumerate((base, v7, ref)):
        ...
        if os.path.exists(src):
            ...
            d.text((x+CELL//2, y+CELL//2), "—", fill=(160,160,170), font=fh, anchor="mm")

out = os.path.join(OUT, "montage_base_v7_004.png")
img.save(out)
