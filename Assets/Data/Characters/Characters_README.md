...
> **代码层命名约定（与中文显示名区分）**：资源文件名 / `m_Name` / `skillId` 使用英文代码名；HUD 与文档展示仍用中文。改命名时务必同步本表、`Characters.xlsx`、两个生成器（`gen_characters_xlsx.py` / `SkillLibraryGenerator.cs`）三处。

| characterId | 中文显示名 | 代码层文件名 / m_Name | 备注 |
|---:|---|---|---|
| 1 | 小熊（玩家） | `Player_01_Bear` | 玩家自身，编号从 01 起 |
| 2 | 大狗 | `Aibo_1_Bigdog` | 队伍 lane0 |
| 3 | 屎屎 | `Aibo_2_Shit` | 队伍 lane1（曾用名 嘟嘟） |
| 4 | 布姆 | `Aibo_3_Boom` | 队伍 lane2 |
| 5 | 小黑 | `Aibo_4_Black` | 队伍 lane3 |

| 技能中文名 | skillId（代码层） | 归属 |
|---|---|---|
| 全体防御 | `skill_1_team_defense` | 小熊 |
| 全体进攻 | `skill_2_team_offense` | 小熊 |
| 大狗叫 | `skill_3_howl` | 大狗 |
| 美味牛角包 | `skill_4_croissant_heal` | 屎屎 |
| 炸弹雨 | `skill_5_bomb_rain` | 布姆 |
| 断弦高压 | `skill_6_power_surge` | 小黑 |
...
| B | 角色 | `displayName` | 显示名（小熊 / 大狗 / 屎屎 / 布姆 / 小黑） |
...
按飞书原表（玩家：小熊 100/25；队伍：大狗 25/35、屎屎 88/3、布姆 45/20、小黑 68/15），双侧总数值都是：
...
|---|---|---|---|---|---|---|---|
| 1 | 小熊 | 演奏者 | 100 | 25 | 全体防御（获取分数能力下降 80% 来抵御一切负面效果）持续 4s | 无 | 获得 5% 的伤害减少 |
| 2 | 大狗 | | 25 | 35 | （3）大狗叫（将即将出现的音符附魔，每成功完成一个音符增加分贝，结算完后发出狗叫按分贝惊吓对手降低对方连击数）（必杀） | 300 | 狗叫后追加一次狗叫 |
| 3 | 屎屎 | | 88 | 3 | （4）（即将将出现的音符（6 个）附魔，每成功完成一个音符就对自己进行一点生命治愈（3 点生命））（必杀） | 200 | 获得治疗后进入缓慢回复（大招之后每三秒根据收集音符数量 ×(1) 回复生命，持续一段时间（9s）） |
| 4 | 布姆 | | 45 | 20 | （5）炸弹雨（即将将出现的音符（3 个）附魔，每完成一个音符就朝对手随机投射一颗小型炸弹（10 点伤害），造成直接生命伤害直到结算完毕）（必杀） | 280 | 生成更多音符 (+2) |
