...
public static class RuntimeSpriteUtility
{
    ...
    private static Sprite _glowBar;
    /// <summary>程序化柔光长条（高斯胶囊）：长轴两端渐隐、短轴上下渐隐。用作 Slide 连线柔光带，避免贴图平铺的段感。
    /// pivot 居中，材质用 AdditiveGlow，颜色由调用方 SpriteRenderer.color 控制（暖黄柔光）。</summary>
    public static Sprite GlowBar
    {
        get
        {
            if (_glowBar == null)
            {
                ...
                {
                    float ny = (y + 0.5f) / h;
                    float dy = Mathf.Abs(ny - 0.5f) * 2f;        // 0 中心 → 1 上下边
                    float side = 1f - Smooth01(dy * 0.95f);      // 中间亮、上下淡
                    for (int x = 0; x < w; x++)
                    {
                        float nx = (x + 0.5f) / w;
                        float dx = Mathf.Abs(nx - 0.5f) * 2f;     // 0 中心 → 1 两端
                        float edge = 1f - Smooth01(dx * 0.9f);     // 中间亮、两端淡
                        float a = Mathf.Pow(edge * side, 1.2f);
                        px[x + y * w] = new Color(1f, 1f, 1f, a);
                    }
                }
                tex.SetPixels(px);
                tex.Apply(false, true);
                _glowBar = Sprite.Create(tex, new Rect(0, 0, w, h), new Vector2(0.5f, 0.5f), h);
            }
            return _glowBar;
        }
    }
